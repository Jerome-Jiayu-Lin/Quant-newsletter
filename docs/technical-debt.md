...
| Website | B | validated latest fallback, bilingual rendering, versioned history-index loading, localized historical Edition/Card routes, explicit missing/unavailable states, eight data-loader tests, lint, and production build verification | no browser-level accessibility or failure-state test exercises the deployed site |
