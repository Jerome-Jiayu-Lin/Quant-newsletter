...
| Historical delivery | C | R2 ownership, tested publication/recovery contracts, separate production/preview bindings, indexed latest preference, compatibility fallbacks, and localized historical routes are implemented | no preview object has been uploaded and no deployed route has read a real R2 index |
