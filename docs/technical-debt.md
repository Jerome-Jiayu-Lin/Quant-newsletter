...
| Architecture | B | explicit Python dependency direction, runtime ownership, public-platform boundaries, versioned provider-independent publication contracts, AST dependency lint, root placement enforcement, and repair-oriented checker tests | the accepted public-platform adapters have not been implemented or exercised together |
...
| Publication | C | Edition completeness validation, versioned export/index/receipt schemas, sanitization allowlist, deterministic object keys, stable Card IDs, clean temporary-checkout publication, scheduled latest-Edition commits, Worker deployment, and build-before-publish behavior | two publication paths diverge and no provider adapter retains a publication receipt or proves rollback |
| Historical delivery | D | R2 ownership, tested dated Public Export and newest-first history-index contracts, compatibility migration, and ordering constraints are decided | no R2 bucket binding, uploaded dated object, migration integration test, or deployed historical route exists |
