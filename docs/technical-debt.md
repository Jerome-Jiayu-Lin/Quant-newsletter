...
| Architecture | B | explicit Python dependency direction, runtime ownership, public-platform boundaries, a provider-independent conditional publisher, versioned publication contracts, AST dependency lint, root placement enforcement, and repair-oriented checker tests | the R2 adapter has not been implemented or exercised with the publisher |
...
| Publication | C | Edition completeness validation plus a fake-storage-tested publisher enforcing sanitization, deterministic keys, idempotency, upload verification, conditional index replacement, and retained receipt shapes | two publication paths diverge and no real provider adapter retains a receipt or proves rollback |
